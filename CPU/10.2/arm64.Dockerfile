...
ARG BASE_IMAGE=vllm/vllm-openai-cpu:latest-arm64
FROM ${BASE_IMAGE}
...
RUN set -eux; \
    ...
    python -c "import importlib.util, sys; sys.exit('The selected BASE_IMAGE does not contain vLLM. Set BASE_IMAGE_ARM64 or BASE_IMAGE_OVERRIDE to a vLLM-capable image.') if importlib.util.find_spec('vllm') is None else None"
