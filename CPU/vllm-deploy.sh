...
| 5  | <Other/other-of-your-cpu-based-models-id                      | <type>             | <other cpu guidance>        |   
+----+---------------------------------------------------------------+--------------------+-----------------------------+
TABLE
}
...
prompt_inputs() {
  ...
  if [[ -z "${API_KEY}" ]]; then
    API_KEY="$(openssl rand -hex 24 2>/dev/null || date +%s%N | sha256sum | awk '{print $1}')"
  fi
}
...
EOFIGNORE
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[dry-run] oc new-build --name=${APP_NAME} --binary --strategy=docker --to=${APP_NAME}:${IMAGE_TAG} -n ${NAMESPACE}"
    echo "[dry-run] oc start-build ${APP_NAME} --from-dir=${WORKDIR} --follow -n ${NAMESPACE}"
  else
    oc new-build --name="${APP_NAME}" --binary --strategy=docker --to="${APP_NAME}:${IMAGE_TAG}" -n "${NAMESPACE}"
