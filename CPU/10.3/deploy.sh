...
generate_vllm_api_key() {
  ...
  if command -v openssl >/dev/null 2>&1; then
    ...
    raw="$(od -An -N64 -tx1 /dev/urandom | tr -d ' \n')"
  else
    raw="$(printf '%s-%s-%s-%s' "${RANDOM:-0}" "${RANDOM:-0}" "$(date +%s%N 2>/dev/null || date +%s)" "$$" | sha256sum | awk '{print $1}')"
  fi
  raw="$(printf '%s' "$raw" | tr -dc 'a-f0-9')"
  while (( ${#raw} < 41 )); do
    raw="${raw}$(printf '%s-%s-%s' "${RANDOM:-0}" "$(date +%s%N 2>/dev/null || date +%s)" "$$" | sha256sum | awk '{print $1}')"
    raw="$(printf '%s' "$raw" | tr -dc 'a-f0-9')"
  done
  printf 'sha256~%s-%s_%s' "${raw:0:6}" "${raw:6:29}" "${raw:35:6}"
}

validate_vllm_api_key_format() {
  local key="${1:-}"
  [[ ${#key} -eq 50 ]] || return 1
  [[ "$key" =~ ^sha256~[a-f0-9]{6}-[a-f0-9]{29}_[a-f0-9]{6}$ ]]
}
...
redhat_authentication() {
  if redhat_registry_login >/dev/null 2>&1; then
    log "Successfully logged into Red Hat registry."
  else
    warn "Failed to log into Red Hat registry. If your selected base image is from registry.redhat.io, the build may fail. Check your credentials and network connectivity."
    exit 0
  fi
}
...
validate_dockerfile_syntax_guards() {
  local rel file bad_lines

  [[ -f "${SCRIPT_DIR}/entrypoints/vllm-nonroot-entrypoint.sh" ]] \
    || fail "required entrypoint is missing: entrypoints/vllm-nonroot-entrypoint.sh"

  for rel in amd64.Dockerfile arm64.Dockerfile ppc64le.Dockerfile s390x.Dockerfile; do
    file="${SCRIPT_DIR}/${rel}"
    bad_lines="$(grep -nE '\\[[:space:]]+$' "$file" || true)"
    if [[ -n "$bad_lines" ]]; then
      printf '%s\n' "$bad_lines" >&2
      fail "${rel} has trailing whitespace after a Dockerfile line-continuation backslash. Remove the whitespace before building."
    fi

    grep -q '/usr/local/bin/vllm-nonroot-entrypoint.sh' "$file" \
      || fail "${rel} does not install the OpenShift non-root vLLM entrypoint."
  done
}
...
write_build_context() {
  ...
  mkdir -p "${BUILD_DIR}/entrypoints"
  cp "${SCRIPT_DIR}/entrypoints/vllm-nonroot-entrypoint.sh" "${BUILD_DIR}/entrypoints/"
...
resolve_vllm_api_key_for_runtime() {
  local existing_api_key=""
  local provided_api_key="${VLLM_API_KEY:-}"

  SECRET="${APP_NAME}-secrets"
  SECRET_KEY="${APP_NAME}-vllm-api-key"
  export SECRET SECRET_KEY

  existing_api_key="$(get_vllm_model_credentials || true)"
  if [[ -n "$existing_api_key" ]]; then
    validate_vllm_api_key_format "$existing_api_key" || fail "Existing API key in secret ${APP_NAME}-secrets / key ${SECRET_KEY} does not match the required 50-character sha256~ format. Refusing to rotate it automatically."
    VLLM_API_KEY="$existing_api_key"
    export VLLM_API_KEY
    log "Reusing existing VLLM API key from secret ${APP_NAME}-secrets / key ${SECRET_KEY}; key will not be rotated."
    return 0
  fi

  if [[ -n "$provided_api_key" ]]; then
    validate_vllm_api_key_format "$provided_api_key" || fail "VLLM_API_KEY must match sha256~xxxxxx-xxxxxxxxxxxxxxxxxxxxxxxxxxxxx_xxxxxx and be exactly 50 characters."
    VLLM_API_KEY="$provided_api_key"
    export VLLM_API_KEY
    log "Using VLLM_API_KEY supplied from the environment for first-time secret creation."
    return 0
  fi

  VLLM_API_KEY="$(generate_vllm_api_key)"
  validate_vllm_api_key_format "$VLLM_API_KEY" || fail "Generated VLLM_API_KEY did not match the required 50-character sha256~ format."
  export VLLM_API_KEY
  log "Generated a new VLLM API key for first-time secret creation. Future deployments will reuse the value from the OpenShift Secret."
}
...
main() {
  ...
  if [[ -n "$VLLM_API_KEY" ]]; then
    validate_vllm_api_key_format "$VLLM_API_KEY" || fail "VLLM_API_KEY must match sha256~xxxxxx-xxxxxxxxxxxxxxxxxxxxxxxxxxxxx_xxxxxx and be exactly 50 characters."
  fi
  ...
  mkdir -p "$WORK_DIR"
  write_build_context
  if [[ "${DRY_RUN:-0}" == "1" && -z "${VLLM_API_KEY:-}" ]]; then
    VLLM_API_KEY="$(generate_vllm_api_key)"
    validate_vllm_api_key_format "$VLLM_API_KEY" || fail "Generated VLLM_API_KEY did not match the required 50-character sha256~ format."
    export VLLM_API_KEY
  fi
